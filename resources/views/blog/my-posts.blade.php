...
@section('body')
    <header class="header-bg bg-light border-bottom mb-5 py-5">
        <div class="container text-center">
            <h1 class="fw-bolder">My Posts</h1>
        </div>
    </header>
        ...
        <div class="row g-4 mb-5">
            @foreach ($posts as $post)
                <div class="col-md-6 col-lg-3">
                    <div class="blog-grid h-100 d-flex flex-column">
                        {{-- Image --}}
                        <div class="blog-grid-img">
                            <img class="image" src="{{ asset('images/' . $post->image_path) }}" alt="{{ $post->title }}"
                        ...
                        <div class="blog-grid-text p-4 d-flex flex-column flex-grow-1">
                            ...
                            <div class="mt-3 pt-3 border-top d-flex gap-2">
                                <a href="{{ route('posts.single.post', $post->slug) }}"
                                    class="btn btn-sm btn-outline-success w-100">Show Post</a>
                                <a href="{{ route('posts.edit', $post->slug) }}"
                                    class="btn btn-sm btn-outline-primary w-100">Edit</a>
                                <form action="{{ route('posts.destroy', $post->slug) }}" method="POST" class="w-100">
                                    @csrf
                                    @method('DELETE')
